...
namespace UnityProductivityTools.TaskTool.Editor
{
    public class TaskManagerSyncedWindow : EditorWindow
    {
        ...
        [MenuItem("Tools/GameDevTools/Task Manager (Synced)")]
        ...
        private void BroadcastTasks()
        {
            ...
            string json = JsonUtility.ToJson(_taskData);
        ...
        private void OnGUI()
        {
            ...
            if (GUILayout.Button("Send Test Msg", EditorStyles.toolbarButton))
            {
                WebSocketEditorListener.Send(new WSMessage { 
                    sender = "editor", 
                    type = "status", 
                    payload = "Ping from Editor UI" 
                });
            }

            if (GUILayout.Button("Test Sync", EditorStyles.toolbarButton))
            {
                WebSocketEditorListener.Send(new WSMessage { 
                    sender = "editor", 
                    type = "status", 
        ...
        private void DrawTaskItem(TaskItem task, int index)
        {
            ...
            if (task.IsExpanded)
            {
                ...
                {
                    EditorUtility.SetDirty(_taskData);
                    // We don't broadcast on every keystroke to avoid spamming
                    // but maybe we should add a "Save/Sync" button or use a delay
                }
